...
const BlogCard = ({ blog }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 dark:border-gray-600 p-5 rounded-2xl shadow-lg border hover:scale-105 transition-all">
      {blog?.thumbnail && (
        <img src={blog.thumbnail} alt="thumbnail" className="rounded-lg" />
      )}
      <p className="text-sm mt-2">
        By {blog?.author?.firstName ?? "Unknown"} |{" "}
        {blog?.category ?? "No category"} | {formattedDate}
      </p>
      <h2 className="text-xl font-semibold mt-1">
        {blog?.title ?? "Untitled"}
      </h2>
      <h3 className="text-gray-500 mt-1">{blog?.subtitle ?? "No subtitle"}</h3>
